...
;; CHECK: test:[[# @LINE + 6 ]]:20: warning: use of potentially tainted data
;; CHECK-NEXT:     (stx-transfer? amount (as-contract tx-sender) tx-sender)
;; CHECK-NEXT:                    ^~~~~~
;; CHECK-NEXT: test:[[# @LINE - 4 ]]:25: note: source of taint here
...
;; CHECK-NEXT:                    ^~~~~~~~~~~~~~
;; CHECK-NEXT: test:[[# @LINE - 4 ]]:30: note: source of taint here
...
(define-public (let-tainted (amount uint))
    (let ((x amount))
;; CHECK: test:[[# @LINE + 6 ]]:24: warning: use of potentially tainted data
;; CHECK-NEXT:         (stx-transfer? x (as-contract tx-sender) tx-sender)
;; CHECK-NEXT:                        ^
;; CHECK-NEXT: test:[[# @LINE - 5 ]]:29: note: source of taint here
...
;; CHECK-NOT: test:[[# @LINE + 1 ]]:24: warning:
        (stx-transfer? amount (as-contract tx-sender) tx-sender)
    )
)
...
;; CHECK: test:[[# @LINE + 9 ]]:24: warning: use of potentially tainted data
;; CHECK-NEXT:         (stx-transfer? x (as-contract tx-sender) tx-sender)
;; CHECK-NEXT:                        ^
;; CHECK-NEXT: test:[[# @LINE - 5 ]]:35: note: source of taint here
;; CHECK-NEXT: (define-public (let-tainted-twice (amount1 uint) (amount2 uint))
;; CHECK-NEXT:                                    ^~~~~~~
;; CHECK-NEXT: test:[[# @LINE - 8 ]]:50: note: source of taint here
;; CHECK-NEXT: (define-public (let-tainted-twice (amount1 uint) (amount2 uint))
;; CHECK-NEXT:                                                   ^~~~~~~
        (stx-transfer? x (as-contract tx-sender) tx-sender)
    )
...
;; CHECK-NEXT:         (stx-transfer? x (as-contract tx-sender) tx-sender)
;; CHECK-NEXT:                        ^
;; CHECK-NEXT: test:[[# @LINE - 6 ]]:64: note: source of taint here
...
;; CHECK-NOT: test:[[# @LINE + 1 ]]:40: warning:
...
;; CHECK-NEXT:                    ^~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;; CHECK-NEXT: test:[[# @LINE - 4 ]]:33: note: source of taint here
...
(define-public (and-tainted (amount uint))
    (ok (and
;; CHECK: test:[[# @LINE + 6 ]]:38: warning: use of potentially tainted data
...
(define-public (and-filter (amount uint))
    (ok (and
        (< amount u100)
;; CHECK-NOT: test:[[# @LINE + 1 ]]:38: warning:
        (unwrap-panic (stx-transfer? amount (as-contract tx-sender) tx-sender))
    ))
)
...
;; CHECK-NEXT:         (unwrap-panic (stx-transfer? amount (as-contract tx-sender) tx-sender))
;; CHECK-NEXT:                                      ^~~~~~
;; CHECK-NEXT: test:[[# @LINE - 5 ]]:34: note: source of taint here
...
;; CHECK-NEXT:         (unwrap-panic (stx-transfer? amount (as-contract tx-sender) tx-sender))
;; CHECK-NEXT:                                      ^~~~~~
;; CHECK-NEXT: test:[[# @LINE - 5 ]]:28: note: source of taint here
...
;; CHECK-NEXT:         (unwrap-panic (stx-transfer? amount (as-contract tx-sender) tx-sender))
;; CHECK-NEXT:                                      ^~~~~~
;; CHECK-NEXT: test:[[# @LINE - 5 ]]:33: note: source of taint here
...
;; CHECK: test:[[# @LINE + 6 ]]:16: warning: use of potentially tainted data
;; CHECK-NEXT:     (stx-burn? amount (as-contract tx-sender))
;; CHECK-NEXT:                ^~~~~~
;; CHECK-NEXT: test:[[# @LINE - 4 ]]:34: note: source of taint here
...
;; CHECK: test:[[# @LINE + 6 ]]:25: warning: use of potentially tainted data
...
;; CHECK: test:[[# @LINE + 6 ]]:29: warning: use of potentially tainted data
;; CHECK-NEXT:     (ft-transfer? stackaroo amount (as-contract tx-sender) tx-sender)
;; CHECK-NEXT:                             ^~~~~~
;; CHECK-NEXT: test:[[# @LINE - 4 ]]:37: note: source of taint here
...
;; CHECK: test:[[# @LINE + 6 ]]:27: warning: use of potentially tainted data
...
;; CHECK: test:[[# @LINE + 6 ]]:31: warning: use of potentially tainted data
;; CHECK-NEXT:     (nft-transfer? stackaroo2 amount (as-contract tx-sender) tx-sender)
;; CHECK-NEXT:                               ^~~~~~
;; CHECK-NEXT: test:[[# @LINE - 4 ]]:38: note: source of taint here
...
;; CHECK: test:[[# @LINE + 12 ]]:37: warning: use of potentially tainted data
;; CHECK-NEXT:     (ok (map-set mymap {key-name-1: key} {val-name-1: value}))
;; CHECK-NEXT:                                     ^~~
;; CHECK-NEXT: test:[[# @LINE - 4 ]]:33: note: source of taint here
;; CHECK-NEXT: (define-public (tainted-map-set (key uint) (value int))
;; CHECK-NEXT:                                  ^~~
;; CHECK-NEXT: test:[[# @LINE + 6 ]]:55: warning: use of potentially tainted data
;; CHECK-NEXT:     (ok (map-set mymap {key-name-1: key} {val-name-1: value}))
;; CHECK-NEXT:                                                       ^~~~~
;; CHECK-NEXT: test:[[# @LINE - 10 ]]:44: note: source of taint here
;; CHECK-NEXT: (define-public (tainted-map-set (key uint) (value int))
;; CHECK-NEXT:                                             ^~~~~
...
;; CHECK: test:[[# @LINE + 12 ]]:40: warning: use of potentially tainted data
;; CHECK-NEXT:     (ok (map-insert mymap {key-name-1: key} {val-name-1: value}))
;; CHECK-NEXT:                                        ^~~
;; CHECK-NEXT: test:[[# @LINE - 4 ]]:36: note: source of taint here
;; CHECK-NEXT: (define-public (tainted-map-insert (key uint) (value int))
;; CHECK-NEXT:                                     ^~~
;; CHECK-NEXT: test:[[# @LINE + 6 ]]:58: warning: use of potentially tainted data
;; CHECK-NEXT:     (ok (map-insert mymap {key-name-1: key} {val-name-1: value}))
;; CHECK-NEXT:                                                          ^~~~~
;; CHECK-NEXT: test:[[# @LINE - 10 ]]:47: note: source of taint here
;; CHECK-NEXT: (define-public (tainted-map-insert (key uint) (value int))
;; CHECK-NEXT:                                                ^~~~~
...
;; CHECK: test:[[# @LINE + 6 ]]:40: warning: use of potentially tainted data
